...
desc('koaRoutr#put')
.should('create `put` handle', function (t) {
  const app = koa()
  ...
  const s = app.listen(function () {
    makeRequest(s.address(), { method: 'PUT', path: '/abc'}, fn).end('HEY!')
...
.should('create `put` handling params', function (t) {
  ...
  const s = app.listen(function () {
    makeRequest(s.address(), { method: 'PUT', path: '/abc/123'}, fn).end('HEY!')
    function fn(res, body) {
      t.equals(res.statusCode, 200)
      t.equals(body, 'OK!!!')
      s.close(function () { t.end() })
...
.should('handle middleware before `put`', function (t) {
  const app = koa()
  const router = koaRoutr()
  ...
  const s = app.listen(function () {
    makeRequest(s.address(), { method: 'PUT', path: '/abc'}, fn).end()
    function fn(res, body) {
      t.equals(res.statusCode, 200)
